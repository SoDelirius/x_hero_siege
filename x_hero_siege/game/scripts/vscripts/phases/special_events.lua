...
function RameroAndBaristolEvent(time) -- 500 kills
	...
	timers.RameroAndBaristol = Timers:CreateTimer(time, function() -- Teleport back to the spawn
		...
		-- I think we should just find them by name and remove them (i.e. point_special_arena_1) instead of using a global variable. Delirius
		-- I was also looking into where you make these and it seems like you specificaly create them using local variables so does this do anything? Delirius
