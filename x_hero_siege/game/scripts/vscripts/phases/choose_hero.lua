...
function SpawnHeroesBis()
	...
	Timers:CreateTimer(5.0, function()
		if hero_vip_count == 4 then
			-- Changed these into a loop. Delirius
			for _, unit_name in pairs({"npc_dota_hero_chaos_knight", "npc_dota_hero_keeper_of_the_light"}) do
				local dummy_hero = CreateUnitByName(unit_name, Entities:FindByName(nil, "choose_vip_4_point"):GetAbsOrigin() + Vector(-100, 0, 0), true, nil, nil, DOTA_TEAM_GOODGUYS)
		...
		elseif hero_vip_count == 8 then
			for _, unit_name in pairs({"npc_dota_hero_storm_spirit", "npc_dota_hero_ember_spirit", "npc_dota_hero_earth_spirit"}) do
				local dummy_hero = CreateUnitByName("", Entities:FindByName(nil, "choose_vip_8_point"):GetAbsOrigin() + Vector(0, 100, 0), true, nil, nil, DOTA_TEAM_GOODGUYS)
					dummy_hero:SetAngles(0, 270, 0)
					dummy_hero:AddAbility("dummy_passive_vulnerable"):SetLevel(1)
					dummy_hero.is_fake_hero = true
			end
...
function SpawnBosses()
	local RAMERO_DUMMY = CreateUnitByName("npc_ramero", Entities:FindByName(nil, "point_special_arena_1"):GetAbsOrigin(), true, nil, nil, DOTA_TEAM_GOODGUYS)
	RAMERO_DUMMY:SetAngles(0, 270, 0)
	RAMERO_DUMMY:AddNewModifier(_G.RAMERO_DUMMY, nil, "modifier_command_restricted", {})
	
	local BARISTOL_DUMMY = CreateUnitByName("npc_baristol", Entities:FindByName(nil, "point_special_arena_2"):GetAbsOrigin(), true, nil, nil, DOTA_TEAM_GOODGUYS)
	BARISTOL_DUMMY:SetAngles(0, 270, 0)
	BARISTOL_DUMMY:AddNewModifier(_G.BARISTOL_DUMMY, nil, "modifier_command_restricted", {})

	local RAMERO_BIS_DUMMY = CreateUnitByName("npc_ramero_2", Entities:FindByName(nil, "point_special_arena_3"):GetAbsOrigin(), true, nil, nil, DOTA_TEAM_GOODGUYS)
	RAMERO_BIS_DUMMY:SetAngles(0, 270, 0)
	RAMERO_BIS_DUMMY:AddNewModifier(_G.RAMERO_BIS_DUMMY, nil, "modifier_command_restricted", {})
