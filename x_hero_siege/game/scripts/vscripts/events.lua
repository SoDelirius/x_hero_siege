ListenToGameEvent('game_rules_state_change', function()
	...
	if newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
		...
		-- Are we doing this twice? Can we set it to just run once? Delirius
...
ListenToGameEvent('npc_spawned', function(keys)
	...
	-- This seems like playing 8 man X Hero Siege with opening another lane would be significantly easier because it just doubles your gold and XP gain. Delirius
	...
	if npc and IsValidEntity(npc) then
		local innate_abilities = {
			...
			"holdout_spirit_str",
			"holdout_spirit_agi",
			"holdout_yellow_effect", 
			"holdout_blue_effect",
			"holdout_green_effect",
			"holdout_red_effect",
	...
	-- This seems to add the level 20 ability to clones, illusions, and specific modifiers? Delirius
	if npc:IsFakeHero() and AbilitiesHeroes_XX[npc:GetUnitName()] then
		...
		-- This seems like something we would only want to do once. Should the if statement be included in the else above it to be an elseif? Delirius
		...
		-- Can't we just use the value from npc.bFirstSpawned instead of creating a new one? Delirius
		npc.bFirstSpawnComplete = true
		-- The next two lines don't seem to be used anywhere anymore Delirius
	...
	-- Does this accomplish what was intended? It seems like the chaos_knight and keeper of the light would be broken due to not being able to get ability points when respawning if they
	-- had any. 
	if npc.bFirstSpawnComplete == true then
		if npc:GetUnitName() == "npc_dota_hero_chaos_knight" or npc:GetUnitName() == "npc_dota_hero_keeper_of_the_light" then
			npc:SetAbilityPoints(0)
	...
	if not npc:IsRealHero() and (npc:GetTeamNumber() == DOTA_TEAM_CUSTOM_1 or npc:GetTeamNumber() == DOTA_TEAM_CUSTOM_2 or npc:GetTeamNumber() == DOTA_TEAM_NEUTRALS) then
		...
		-- I think the following could be quicker than above. Delirius
		local percentages =	{
			{ 1.5, 1.25, 0.75 },
			{ 1.1, 1, 1 },
			{ 1, 0.9, 1.25 },
		...
		npc:SetMinimumGoldBounty(normal_bounty * percentages[difficulty][1])
		npc:SetMaximumGoldBounty(normal_bounty * percentages[difficulty][1])
		npc:SetDeathXP(normal_xp * percentages[difficulty][2])
		npc:SetBaseDamageMin(normal_min_damage * percentages[difficulty][3])
		npc:SetBaseDamageMax(normal_max_damage * percentages[difficulty][3])
...
ListenToGameEvent('dota_player_learned_ability', function(keys)
	...
	-- Why isn't doombringer allowed to level up these abilities? Delirius
	...
	-- Why issue are we seeing with the lone druid bears? This seems like it could be an extremely expensive operation to perform. Delirius
...
ListenToGameEvent('dota_player_gained_level', function(keys)
	...
	-- What was this in use for? Delirius
	...
	-- Condensed this section down for readability. Delirius
	if hero_level == 20 then
		-- Why are we removing these abilities at Level 20? Delirius
		...
		-- Is this fix still needed, should we change this to on hero creation instead of checking at level 20? Delirius
		-- 7.23 outpost capture ability fix
		if hero:HasAbility("ability_capture") then
			hero:RemoveAbility("ability_capture")
		end

		-- Why are we checking for this specifically at level 20? Delirius
		for i = 0, 17 do
		...
		-- This section is weird to me. I understand we are adding abilities to the hero at Level 20. Why is it done in this way? Why do some heroes has 4 level 20 abilities that seem
		-- to just be a different version of their normal abilities? Delirius
		if AbilitiesHeroes_XX[hero:GetUnitName()] then
			print("Whisper Level 20 Ability")
			-- I don't think that this is used anywhere as the function that requires hero.lvl_20 to be set is never called. Delirius
			hero.lvl_20 = true
...
ListenToGameEvent('entity_killed', function(keys)
	-- Condensed variables for readability Delirius
	...
	-- Returns nothing for errors and containers Delirius
	...
	-- Not reference anywhere Delirius
	-- if keys.entindex_inflictor ~= nil then killerAbility = EntIndexToHScript(keys.entindex_inflictor) end Delirius
	...
	if killedUnit:IsRealHero() and (killedUnit:GetTeamNumber() == DOTA_TEAM_GOODGUYS) then
		...
		-- What are we trying to fix here? Delirius
		...
		-- Changed to single if statement and removed else Delirius
		if CustomTimers.game_phase == 3 and killedUnit.ankh_respawn == false then
	...
	elseif killedUnit:IsCreature() then
		-- Im not understanding what this variable is for. The two different Ramero fights are in different events and so their timers should not be together. Delirius
		-- Also to add to this. ramero_check is a local variable so it will never be able to add up to two because different rameros will be killed at different times Delirius
		...
		elseif killedUnit:GetUnitName() == "npc_baristol" then
			...
			-- Cleaned this up a bit by removing second if statement Delirius
		...
		elseif killedUnit:GetUnitName() == "npc_ramero_2" then
			...
			-- This doesn't seem to be used anywhere Delirius
		...
		elseif killedUnit:GetUnitName() == "npc_dota_creature_muradin_bronzebeard" and killedUnit:GetTeamNumber() ~= 2 then
		...
		-- This could be made cleaner by just using if MAGTHERIDON == difficulty. I know it changes it a little but you could add and difficulty > 3 for extreme and divine difficulty to add more Delriius
		elseif killedUnit:GetUnitName() == "npc_dota_hero_magtheridon" then
		...
		-- Could we do killer.IsPlayer() and killer.IsOwnedByAnyPlayer() here to simplify all of this? Delirius
		if killer:IsRealHero() or killer:IsIllusion() or IsValidEntity(killer:GetPlayerOwner()) then
			if killer:GetTeamNumber() == 2 then
				-- Why are we checking for their team if there is only two teams? Delirius
				if killedUnit:GetTeamNumber() == 6 then
					if killer:IsIllusion() then
						-- Changed to local owner so that it is more clear Delirius
						local owner = PlayerResource:GetSelectedHeroEntity(killer:GetPlayerOwnerID())

						owner:IncrementKills(1)

						for _, Zone in pairs(GameMode.Zones) do
							if Zone:ContainsUnit(owner) then
								Zone:AddStat(owner:GetPlayerID(), ZONE_STAT_KILLS, 1)
							end
						end

					-- Wouldn't this section work for illusions as well because you are getting the owning player ID Delirius
					...
					-- This could cause issues because of where the  if statement for this is located. It should apply to both illusions and other entities. Delirius
					-- reward system based on kills, including kill events
					-- Changed to be == and have correct values. Delirius
					...
					elseif killer:GetKills() == 500 and RAMERO == 0 then
						StartRameroAndBaristolEvent(killer)
					elseif killer:GetKills() == 750 and RAMERO == 1 then
	...
	elseif killedUnit:IsBuilding() then
		...
		-- Is this not the same as above but when it dies it ends the game? Delirius
...
-- This function exists twice. Which one is the correct one? Delirius
...
-- Just commenting this out Delirius
...
-- Is this function used for anything? Delirius
-- This function exists twice. Delirius
...
function GameMode:OnPlayerHeroEnteredZone(playerHero, zoneName)
	-- This seems to ask for a lot of information when we just need a bool. Could we use IsPlayer() instead Delirius
...
-- This seems to call itself? Or maybe its called CDungeonZone:OnZoneActivated.  But it could probably be made more clear. Delirius
-- These functions seem to have a lot of nesting and it could probably be cleaned up. This might be my lack of experience but I dont understand why you would create and call a function here
-- to just call another function somewhere else in a loop. Delirius
